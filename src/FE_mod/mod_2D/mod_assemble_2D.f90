module mod_assemble_2D
   ...
   !> Interface for assembling dense and sparse matrix
   interface assemble_matirx_2D
      module procedure assemble_dense_matirx_2D
      module procedure assemble_sparse_COO_matirx_2D
   end interface

contains
   !! triangle_fe
   subroutine assemble_dense_matirx_2D(A, co_func, lb_func, &
   ...
   end subroutine assemble_dense_matirx_2D

   subroutine assemble_sparse_COO_matirx_2D(sparseA, co_func, lb_func, &
      M_partition, T_partition, &
      T_basis_trial, T_basis_test, &
      der_x_trial, der_y_trial, &
      der_x_test, der_y_test, &
      field_info)
      !> Alogrithm I-3
      type(sparse_COO), intent(inout) :: sparseA
      procedure(cofunc_2d), pointer, intent(in) :: co_func
      !! co_func: The coefficient function
      procedure(local_basis_func), pointer, intent(in) :: lb_func
      real(wp), intent(in)    :: M_partition(:, :)
         ...
         do index_trial = 1, field_info%number_of_local_basis_trial
            do index_test = 1, field_info%number_of_local_basis_test
               ...
               call quadfunc%initialize(afunc, &
                  ...
                  field_info%Gauss_point_number)
               call quadfunc%integrate(ans, ierr, err)
               !> The sparse matrix is stored in the COO format
               call sparseA%append(&
                  T_basis_test(index_test, index_n), &
                  T_basis_trial(index_trial, index_n),&
                  ans)
            end do
         end do
      end do
   end subroutine assemble_sparse_COO_matirx_2D
      ...
      do index_n = 1, field_info%number_of_elements
         ...
         do index_test = 1, field_info%number_of_local_basis_test
            ...
            call quadfunc%initialize(bfunc, &
               ...
               field_info%Gauss_point_number)
            call quadfunc%integrate(ans, ierr, err)
            b(T_basis_test(index_test, index_n)) = &
               b(T_basis_test(index_test, index_n)) + ans
         end do
